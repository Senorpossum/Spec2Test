...
export class CodebaseAnalyzer {
    ...
    private detectLanguage(): Language {
        ...
        for (const [lang, exts] of Object.entries(extensions) as [Language, string[]][]) {
            ...
            counts[lang] = exts.reduce(
                (sum, ext) => sum + (extCount[ext] || 0),
                0,
            );
        }
    ...
    private identifyCoverageGaps(
        ...
        const hasSecurityTests = existingTests.some((t) =>
        ...
        if (!hasSecurityTests) {
            gaps.push({
                area: 'security',
                description: 'No security tests detected. Consider adding authentication, authorization, and input validation tests.',
                affectedModules: [],
                priority: 'critical',
            });
        }
    ...
    private walkDirectory(
        dir: string,
        callback: (filePath: string, dirName: string) => void,
    ): void {
        try {
